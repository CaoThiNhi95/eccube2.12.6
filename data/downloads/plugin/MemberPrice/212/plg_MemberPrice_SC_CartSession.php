    ...
    function getCartList($productTypeId)
    {
        ...
        for ($i = 0; $i <= $max; $i++) {
            if (isset($this->cartSession[$productTypeId][$i]['cart_no']) && $this->cartSession[$productTypeId][$i]['cart_no'] != '') {
                ...
                if ($objCustomer->isLoginSuccess(true) === true && !is_null($this->cartSession[$productTypeId][$i]['productsClass']['plg_memberprice_price03']) && $this->cartSession[$productTypeId][$i]['productsClass']['plg_memberprice_price03'] != '') {

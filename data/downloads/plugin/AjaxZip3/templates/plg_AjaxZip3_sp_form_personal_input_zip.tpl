<dd>
    ...
    <!--{/if}-->
    <p><input type="tel" name="<!--{$key1}-->" value="<!--{$arrForm[$key1]|h}-->" max="<!--{$smarty.const.ZIP01_LEN}-->" class="boxShort text data-role-none" onkeyup="fnCallAddress('', '<!--{$key1}-->','<!--{$key2}-->','<!--{$key3}-->','<!--{$key4}-->', true);" />&nbsp;－&nbsp;<input type="tel" name="<!--{$key2}-->" value="<!--{$arrForm[$key2]|h}-->" maxlength="<!--{$smarty.const.ZIP02_LEN}-->" class="boxShort text data-role-none" onkeyup="fnCallAddress('', '<!--{$key1}-->','<!--{$key2}-->','<!--{$key3}-->','<!--{$key4}-->', true);" />&nbsp;&nbsp;<a href="http://search.post.japanpost.jp/zipcode/" target="_blank" rel="external"><span class="fn">郵便番号検索</span></a></p>
</dd>

 ...
 * @version $Id: AjaxZip3.php 680 2015-07-21 06:14:06Z habu $
 */
        ...
        switch ($device_type_id) {
            case DEVICE_TYPE_PC:
                ...
                if ($this->endsWith($filename, 'frontparts/form_personal_input.tpl') !== false) {
                    $objTransform->select('tr',2)->replaceElement(file_get_contents($template_dir . 'plg_AjaxZip3_pc_form_personal_input_zip.tpl'));
                }
                // お問い合わせ画面
                if ($this->endsWith($filename, 'contact/index.tpl') !== false) {
                    $objTransform->select('tr',2)->replaceElement(file_get_contents($template_dir . 'plg_AjaxZip3_pc_form_contact_input_zip.tpl'));
                }
                // 非会員購入画面
                if ($this->endsWith($filename, 'shopping/nonmember_input.tpl') !== false) {
                    $objTransform->select('tr',2)->replaceElement(file_get_contents($template_dir . 'plg_AjaxZip3_pc_form_nonmember_input_zip.tpl'));
                }
                break;
            case DEVICE_TYPE_SMARTPHONE:
                // 会員登録画面
                if ($this->endsWith($filename, 'frontparts/form_personal_input.tpl') !== false) {
                    $objTransform->select('dd',2)->replaceElement(file_get_contents($template_dir . 'plg_AjaxZip3_sp_form_personal_input_zip.tpl'));
                }
                // お問い合わせ画面
                if ($this->endsWith($filename, 'contact/index.tpl') !== false) {
                ...
                // 非会員購入画面
                if ($this->endsWith($filename, 'shopping/nonmember_input.tpl') !== false) {
                    $objTransform->select('dd',2)->replaceElement(file_get_contents($template_dir . 'plg_AjaxZip3_sp_form_nonmember_input_zip.tpl'));
                }
                break;
            case DEVICE_TYPE_ADMIN:
            default:

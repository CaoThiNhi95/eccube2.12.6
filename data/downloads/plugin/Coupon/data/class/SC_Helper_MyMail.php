 ...
 *
 */
class SC_Helper_MyMail extends SC_Helper_Mail {
    ...
    /* 受注完了メール送信 */
    function sfSendOrderMail($order_id, $template_id, $subject = '', $header = '', $footer = '', $send = true, $coupon = false) {
        ...
        if($template_id==2){
            $path = PLUGIN_UPLOAD_REALDIR . "Coupon/template/mobile/mail_templates/plg_Coupon_order_mail.tpl";
        }else{
            $path = PLUGIN_UPLOAD_REALDIR . "Coupon/template/default/mail_templates/plg_Coupon_order_mail.tpl";
        }
        
        $body = $objMailView->fetch($path);

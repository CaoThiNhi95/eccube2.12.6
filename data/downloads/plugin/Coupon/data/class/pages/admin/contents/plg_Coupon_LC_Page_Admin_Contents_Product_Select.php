...
/**
 * 商品選択(クーポン管理用) のページクラス.
 *
 * @package Page
 * @author LOCKON CO.,LTD.
 * @version $Id: plg_Coupon_LC_Page_Admin_Contents_Product_Select.php 20970 2011-06-10 10:27:24Z Seasoft $
 */
class plg_Coupon_LC_Page_Admin_Contents_Product_Select extends LC_Page_Admin_Ex {
    ...
    function init() {
        parent::init();
        $this->tpl_mainpage = 'contents/plg_Coupon_product_select.tpl';
    ...
    function lfGetProducts(&$objDb) {
        ...
        //商品コードの表示に関して
        foreach($arrProducts as $key=>$data){
            if($data["product_code_min"]!=$data["product_code_max"]){
                $arrProducts[$key]["product_code"] = $data["product_code_min"]." ～ ".$data["product_code_max"];
            }else{
                $arrProducts[$key]["product_code"] = $data["product_code_max"];
            }
        }
